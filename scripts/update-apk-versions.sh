...
ALPINE_VERSION_FULL=$(grep '^FROM alpine:' "$DOCKERFILE" | head -n1 | sed -E 's/FROM alpine:([^[:space:]]+).*/\1/')
...
# Filter out packages with variable references (containing ${ }) and shell
# syntax tokens ("; fi" from arch-conditional apk add blocks) - only
# pkg=version tokens are pins.
...
extract_new_version()
{
    local url="$1"
    local html
    # || true: a transient fetch failure must report "not found", not kill
    # the whole run via set -e.
    html=$(curl -s "$url") || html=""
    local version
...
# tgstream builds linux/amd64 + linux/arm64 only (Alpine chromium coverage)
...
missing_arches_for_version()
{
    ...
    local missing=""
    # The caller loops packages under IFS=<newline>; restore word splitting
    # locally or BUILD_ARCHES iterates as a single string.
    local saved_ifs="$IFS"
    IFS=' '
    for arch in $BUILD_ARCHES; do
        [ "$arch" = "x86_64" ] && continue    # source of $version, already known
        local v
        v=$(extract_new_version "https://pkgs.alpinelinux.org/package/v${ALPINE_BRANCH}/${repo}/${arch}/${pkg}") || v=""
        # A package absent from this arch entirely (e.g. intel-media-driver
        # on aarch64) is arch-conditional in the Dockerfile, not "missing" -
        # holding back its updates forever would be wrong.
        [ -z "$v" ] && continue
        if [ "$v" != "$version" ]; then
